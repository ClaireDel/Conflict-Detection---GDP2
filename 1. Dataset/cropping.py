...
def main():
    ...
    for filename in os.listdir(folder_in):
        ...
        # Black background
        image_b = np.zeros((img.shape[0], img.shape[1], 3), np.uint8)
        image_b[:] = (0, 0, 0) 
            ...
            for box in pred_boxes:
                ...
                if len(pose_preds)>=1:
                    for kpt in pose_preds:
                        ...
                        draw_pose(kpt,image_b)  # draw the poses on the blackbackground
                
                # Plot the BB on the image 
                # cv2.rectangle(image_b, (int(box[0][0]),int(box[0][1])), (int(box[1][0]),int(box[1][1])) , color=(0, 255, 0),thickness=3)
                
                # Cropping (black background)
                crop_b = image_b.copy()
                crop_b = crop_b[int(box[0][1]):int(box[1][1]), int(box[0][0]):int(box[1][0])]
